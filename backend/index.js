...
app.post("/register", async (req, res) => {
  ...
  try {
    ...
    const sql =
      "INSERT INTO `users`(`nom`, `email`, `mot_de_passe`, `téléphone`, `role_id`, `date_inscription`) VALUES (?,?,?,?,?,?)";
    db.query(
      sql,
      [name, email, hashedpassword, phone, role_id, date_inscription],
      (err, data) => {
        if (err) {
          console.error(err);
...
app.put("/updateuser/:id", (req, res) => {
  ...
  const sql =
    "UPDATE `users` SET`nom`=?,`email`=?,`mot_de_passe`=?,`téléphone`=?,`role_id`=?,`date_inscription`=? where `id` = ?";
  db.query(
    sql,
    [name, email, password, phone, role_id, date_inscription, id],
    (err, msg) => {
      if (err) {
...
app.post("/createexpertises", (req, res) => {
  ...
  const sql =
    "INSERT INTO `expertises`(`sinistre_id`, `expert`, `rapport`, `date_evaluation`) VALUES (?,?,?,?)";
  db.query(sql, [sinistre_id, expert, rapport, date_evaluation], (err, msg) => {
    if (err) {
      return res
        .status(500)
        .json({ message: "Error creating expertises" }, err);
    }
    return res.status(201).json({ message: "Insert Successfull" }, msg);
  });
});
...
app.put("/updateexpertises/:id", (req, res) => {
  ...
  const { id } = req.params;
  const sql =
    "UPDATE `expertises` SET `sinistre_id`=?,`expert`=?,`rapport`=?,`date_evaluation`=? WHERE id=?";
  db.query(
    sql,
    [sinistre_id, expert, rapport, date_evaluation, id],
    (err, msg) => {
      if (err) {
        return res
          .status(500)
          .json({ message: "Error updating expertises" }, err);
      }
      return res.status(201).json({ message: "Update Successfull" }, msg);
    }
  );
...
app.delete("/deleteexpertises/:id", (req, res) => {
  ...
  db.query(sql, [id], (err, msg) => {
    if (err) {
      return res
        .status(500)
        .json({ message: "Error deleting expertises" }, err);
    }
    return res.status(201).json({ message: "deleted Successfully" }, msg);
  });
});
...
app.get("/viewdocuments", (req, res) => {
  ...
  db.query(sql, (err, data) => {
    ...
    const formatted = data.map((doc) => ({
      ...doc,
      contenu_fichier: doc.contenu_fichier
        ? doc.contenu_fichier.toString("base64")
        : null,
    }));
...
app.post("/createdocuments", upload.single("contenu_fichier"), (req, res) => {
  const { sinistre_id, expert, nom_fichier, type_document, date_upload } =
    req.body;
  ...
  db.query(
    ...
    (err, result) => {
      if (err) {
        ...
        return res
          .status(500)
          .json({ message: "Error creating document", error: err });
      }
  ...
  (req, res) => {
    ...
    db.query(sql, values, (err, msg) => {
      if (err)
        return res
          .status(500)
          .json({ message: "Error updating document", error: err });
...
app.delete("/deletedocuments/:id", (req, res) => {
  ...
  db.query(sql, [id], (err, msg) => {
    if (err)
      return res
        .status(500)
        .json({ message: "Error deleting document", error: err });
...
app.get("/download/:id", (req, res) => {
  const { id } = req.params;
  const sql =
    "SELECT nom_fichier, contenu_fichier, type_document FROM documents WHERE id=?";
  db.query(sql, [id], (err, results) => {
    ...
    res.setHeader(
      "Content-Disposition",
      `attachment; filename=${doc.nom_fichier}`
    );
    res.setHeader(
      "Content-Type",
      doc.type_document || "application/octet-stream"
    );
...
app.post("/createpaiements", (req, res) => {
  ...
  const sql =
    "INSERT INTO `paiements`(`sinistre_id`, `montant`, `date_paiement`, `méthode`, `statut`) VALUES (?,?,?,?,?)";
  db.query(
    sql,
    [sinistre_id, montant, date_paiement, méthode, statut],
    (err, msg) => {
      if (err) {
        return res
          .status(500)
          .json({ message: "Error creating paiements" }, err);
      }
      return res.status(201).json({ message: "Insert Successfull" }, msg);
    }
  );
...
app.put("/updatepaiements/:id", (req, res) => {
  ...
  const { id } = req.params;
  const sql =
    "UPDATE `paiements` SET `sinistre_id`=?,`montant`=?,`date_paiement`=?,`méthode`=?,`statut`=? WHERE id=?";
  db.query(
    sql,
    [sinistre_id, montant, date_paiement, méthode, statut, id],
    (err, msg) => {
      if (err) {
        return res
          .status(500)
          .json({ message: "Error updating paiements" }, err);
      }
      return res.status(201).json({ message: "Update Successfull" }, msg);
    }
  );
...
app.post("/createpolices", (req, res) => {
  const { numero_police, utilisateur_id, type, date_debut, date_fin, statut } =
    req.body;
  const sql =
    "INSERT INTO `polices`(`numero_police`, `utilisateur_id`, `type`, `date_debut`, `date_fin`, `statut`) VALUES (?,?,?,?,?,?)";
  db.query(
    sql,
    [numero_police, utilisateur_id, type, date_debut, date_fin, statut],
    (err, msg) => {
      if (err) {
...
app.put("/updatepolices/:id", (req, res) => {
  ...
  const { id } = req.params;
  const sql =
    "UPDATE `polices` SET `numero_police`=?,`utilisateur_id`=?,`type`=?,`date_debut`=?,`date_fin`=?,`statut`=? WHERE id=?";
  db.query(
    sql,
    [numero_police, utilisateur_id, type, date_debut, date_fin, statut, id],
    (err, msg) => {
      if (err) {
...
app.post("/createsinistres", (req, res) => {
  ...
  const sql =
    "INSERT INTO `sinistres`(`Numero_Sinistre`,`utilisateur_id`, `police_id`, `expert`, `date_declaration`, `type`, `description`, `statut`, `montant_requis`, `montant_approuvé`) VALUES (?,?,?,?,?,?,?,?,?,?)";
  db.query(
    sql,
    [
      Numero_Sinistre,
    ...
    (err, msg) => {
      if (err) {
        return res
          .status(500)
          .json({ message: "Error creating sinistres" }, err);
      }
      return res.status(201).json({ message: "Insert Successfull" }, msg);
    }
  );
...
app.put("/updatesinistres/:id", (req, res) => {
  ...
  const { id } = req.params;
  const sql =
    "UPDATE `sinistres` SET `Numero_Sinistre`=?,`utilisateur_id`=?,`police_id`=?,`expert`=?,`date_declaration`=?,`type`=?,`description`=?,`statut`=?,`montant_requis`=?,`montant_approuvé`=? WHERE id=?";
  db.query(
    sql,
    [
      Numero_Sinistre,
    ...
    (err, msg) => {
      if (err) {
        return res
          .status(500)
          .json({ message: "Error updating sinistres" }, err);
      }
      return res.status(201).json({ message: "Update Successfull" }, msg);
    }
  );
